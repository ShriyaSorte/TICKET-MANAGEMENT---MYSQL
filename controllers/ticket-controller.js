...
function addticket() {
  const {
    tickettype, tstatus, ttitle, tdiscription, tattachment, tallocatedid, tremarks, tcreatedby, tmodifiedby,} = req.body;
  ...
  db.query(
    query,
    [tickettype,tstatus,ttitle,tdiscription,tattachment,tallocatedid,tremarks,tcreatedby,tmodifiedby,],(err, result) => {
      if (err) {
        throw err;
      }
      res.send(result);
...
function deleteticket() {
  try {
    ...
    db.query(
      query,
      [ticketid, tickettype, tstatus, ttitle, tdiscription, tattachment, tallocatedid, tremarks, tcreatedby, tmodifiedby],(err, result)=>{
        if (err) throw err;
        res.send(result);
      }
    );
